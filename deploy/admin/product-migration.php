      ...
      <div class="card-body">
        ...
        <div class="pm-row" style="margin-top:16px;">
          ...
          <div class="form-group">
            ...
            <input type="number" id="pmChunk" class="form-control" value="25" min="1" max="100">
            <div class="pm-help">Lower is safer for slow hosts; higher is faster.</div>
          </div>
        </div>
...
async function processCurrentJob() {
  ...
  isProcessing = true;
  const chunk = Math.max(1, Math.min(100, +document.getElementById('pmChunk').value || 25));
  try {
    while (currentJobId) {
