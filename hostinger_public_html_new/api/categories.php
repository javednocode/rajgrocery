...
function getCategories($db) {
    ...
    if (!$isAdmin && $countryParam !== '') {
        ...
        // If country code not found, show all categories (don't return empty)
    }
    ...
    // Batch load product counts — COUNTRY-AWARE
    // Count only active products that are actually assigned to the viewed country.
    // If no country filter is active (admin or all), count all products.
    if (!empty($categories)) {
        $catIds = array_column($categories, 'id');
        $placeholders = implode(',', array_fill(0, count($catIds), '?'));

        if (!$isAdmin && !empty($countryId)) {
            // Country-specific count: only products linked to this country
            $countSql = "SELECT pc.category_id, COUNT(DISTINCT pc.product_id) as cnt
                         FROM product_categories pc
                         INNER JOIN product_countries pcty ON pcty.product_id = pc.product_id AND pcty.country_id = ?
                         WHERE pc.category_id IN ($placeholders)
                         GROUP BY pc.category_id";
            $countParams = array_merge([(int)$countryId], $catIds);
        } else {
    ...
    if (!$isAdmin) {
        cacheSet($cacheKey, $tree, 60); // 1 minute (short — counts are country-specific)
    }
...
function getFeaturedCategories($db) {
    ...
    if ($countryParam !== '') {
        ...
        // If country not resolved, show all featured categories
    }
