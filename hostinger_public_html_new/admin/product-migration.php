...
async function repairCategories() {
  ...
  // Use currently selected admin country if any
  const adminCountry = getAdminCountry();
  const payload = { limit: 1000 };
  if (adminCountry && adminCountry.id) payload.country_id = adminCountry.id;

  try {
    const res = await api('/product-migration/repair-categories', 'POST', payload);
    const d = res.data || {};
