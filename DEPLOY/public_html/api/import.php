...
// Also increase execution time limit for large imports
@set_time_limit(300);
...
// ═══════════════════════════════════════════════════════════════════════════
// PROCESS — actually import into DB
// ═══════════════════════════════════════════════════════════════════════════
function doProcess($db) {
    ...
    $rows    = parseFile($filePath, $fileExt);
    if (!$rows) errorResponse('Cannot parse file', 422);

    $headers = normaliseHeaders(array_shift($rows));
    ...
    foreach ($rows as $idx => $row) {
        ...
        if (($idx + 1) % $batchSize === 0) {
            // In a real chunked scenario you'd store progress to DB/file.
            // For single-request processing, we just continue.
        }
    }
    ...
    ], "Import complete: $imported imported, $updated updated, $skipped skipped, $errors errors");
}
