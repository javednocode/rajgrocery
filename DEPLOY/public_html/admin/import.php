...
<div id="stepImport">
  <div class="card"><div class="card-body">
    ...
    <div style="margin-top:8px;font-size:12px;color:var(--admin-text-muted);">
      ⚠️ Do not close this page. Images are being downloaded in the background.
    </div>
...
async function startImport() {
  setStage(3);
  setProgress(5, 'Sending to server...');
  ...
  try {
    // Progress animation while waiting (server-side is synchronous)
    let fakeProgress = 10;
    const ticker = setInterval(() => {
      fakeProgress = Math.min(fakeProgress + (Math.random() * 3), 85);
      setProgress(fakeProgress, 'Importing & downloading images...');
    }, 800);
