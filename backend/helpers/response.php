...
function applyApiCacheHeaders($statusCode) {
    ...
    if (preg_match('#^/api/(settings|banners)/?#', $uri)) {
...
function paginatedResponse($data, $total, $page, $perPage, array $extraMeta = []) {
    $response = [
        'success' => true,
        'data' => $data,
        'pagination' => [
    ...
    // Merge any extra metadata (like alerts, aggregations, etc) at the top level
    // We use a generic 'meta' key to avoid conflicting with pagination or data
    $response['meta'] = array_merge($response['pagination'], $extraMeta);
