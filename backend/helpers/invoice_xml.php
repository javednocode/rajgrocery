<?php
/**
 * Asian Food Cork - XML Invoice Generator (UBL 2.1 format)
 */

function generateXMLInvoice($order, $items) {
    ...
    $el($invoice, 'cbc', 'DocumentCurrencyCode', 'EUR');
    ...
    $el($partyName, 'cbc', 'Name', 'Asian Food Cork');
    $postalAddr = $xml->createElement('cac:PostalAddress');
    $party->appendChild($postalAddr);
    $el($postalAddr, 'cbc', 'CityName', 'Cork');
    $el($postalAddr, 'cbc', 'CountrySubentity', 'Cork');
    $country = $xml->createElement('cac:Country');
    $postalAddr->appendChild($country);
    $el($country, 'cbc', 'IdentificationCode', 'IE');
    $contact = $xml->createElement('cac:Contact');
    $party->appendChild($contact);
    $el($contact, 'cbc', 'ElectronicMail', 'orders@asianfoodcork.com');
    $el($contact, 'cbc', 'Telephone', '+35321000000');
    ...
    $el($cCountry, 'cbc', 'IdentificationCode', 'IE');
    ...
    $el($taxTotal, 'cbc', 'TaxAmount', number_format($order['tax'] ?? 0, 2), ['currencyID' => 'EUR']);
    $taxSubtotal = $xml->createElement('cac:TaxSubtotal');
    $taxTotal->appendChild($taxSubtotal);
    $el($taxSubtotal, 'cbc', 'TaxableAmount', number_format($order['subtotal'] ?? 0, 2), ['currencyID' => 'EUR']);
    $el($taxSubtotal, 'cbc', 'TaxAmount', number_format($order['tax'] ?? 0, 2), ['currencyID' => 'EUR']);
    ...
    $el($lmt, 'cbc', 'LineExtensionAmount', number_format($order['subtotal'] ?? 0, 2), ['currencyID' => 'EUR']);
    $el($lmt, 'cbc', 'TaxExclusiveAmount', number_format($order['subtotal'] ?? 0, 2), ['currencyID' => 'EUR']);
    $el($lmt, 'cbc', 'TaxInclusiveAmount', number_format($order['total'] ?? 0, 2), ['currencyID' => 'EUR']);
    $el($lmt, 'cbc', 'AllowanceTotalAmount', number_format($order['discount'] ?? 0, 2), ['currencyID' => 'EUR']);
    $el($lmt, 'cbc', 'ChargeTotalAmount', number_format($order['shipping_charge'] ?? 0, 2), ['currencyID' => 'EUR']);
    $el($lmt, 'cbc', 'PayableAmount', number_format($order['total'] ?? 0, 2), ['currencyID' => 'EUR']);
    ...
    foreach ($items as $i => $item) {
        ...
        $el($line, 'cbc', 'LineExtensionAmount', number_format($item['total'], 2), ['currencyID' => 'EUR']);
        ...
        $el($price, 'cbc', 'PriceAmount', number_format($item['price'], 2), ['currencyID' => 'EUR']);
    }
