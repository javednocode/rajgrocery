...
<div class="settings-grid">
    <div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <p style="font-size:13px;color:var(--admin-text-muted);margin:0 0 14px;">
                    If category counts or products are showing stale data, clear the site cache instantly.
                </p>
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    <label style="font-weight:700;font-size:13px;color:var(--admin-text);margin-bottom:10px;display:block">
                        🇾 Brand Logos Manager
                        <span style="font-weight:400;font-size:11.5px;color:var(--admin-text-muted);margin-left:6px">Upload a logo image for each brand. Images will display in the homepage brands carousel.</span>
                    </label>

                    <!-- Brand rows -->
                    <div id="brandRowsWrap" style="display:flex;flex-direction:column;gap:10px;margin-bottom:12px;"></div>

                    <!-- Add brand button -->
                    <button type="button" onclick="brandAddRow()" style="
                        display:inline-flex;align-items:center;gap:7px;
                        background:var(--admin-accent,#1E88A8);color:#fff;
                    ...
                    <!-- Hidden field carries the JSON value -->
                    <input type="hidden" id="featured_brands_data" />
                    <!-- Keep legacy list in sync -->
                    <input type="hidden" id="featured_brands_list" />
                </div>
                <div class="form-row-3">
                ...
                <?php for ($i = 1; $i <= 6; $i++): ?>
                    <div style="padding:16px;border:1px solid var(--admin-border);border-radius:12px;margin-bottom:16px;background:rgba(255,255,255,0.02);">
                        ...
                        <div class="form-group">
                            ...
                            <div id="current_review_<?= $i ?>_photo" style="margin-bottom:8px;"></div>
                            <input type="file" id="review_<?= $i ?>_photo_file" class="form-control" accept="image/*">
                        </div>
                    </div>
                <?php endfor; ?>
            </div>
    ...
    <div>
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-group"><label>Free Shipping Above (store currency)</label><input type="number" id="shipping_free_above" class="form-control"></div>
                <div class="form-group"><label>Shipping Charge (store currency)</label><input type="number" id="shipping_charge" class="form-control"></div>
...
const settingFields = [
  ...
  'featured_brands_label','featured_brands_title','featured_brands_link_text','featured_brands_list','featured_brands_data',
...
async function loadSettings() {
    try {
        ...
        [1,2,3,4,5,6].forEach(i => {
            if (s[`review_${i}_photo`]) {
                const el = document.getElementById(`current_review_${i}_photo`);
                if (el) el.innerHTML = `<img src="../${s[`review_${i}_photo`]}" style="max-height:48px;border-radius:50%;object-fit:cover;width:48px;height:48px;">`;
            }
        });
        promoAfterLoad(s);
        // Load brand logo manager
        brandLoadFromSettings(s);
...
/* ════════════════════════════════════════
   BRAND LOGO MANAGER
   Stores data as JSON in featured_brands_data
   Format: [{name:"MDH",image:"/uploads/brands/mdh.png"}, ...]
═══════════════════════════════════════════ */
let _brandRows = [];

function brandLoadFromSettings(s) {
    // Try JSON first (featured_brands_data), fall back to plain list
    let rows = [];
    if (s.featured_brands_data) {
        try {
            const parsed = JSON.parse(s.featured_brands_data);
            if (Array.isArray(parsed)) rows = parsed;
        } catch {}
    }
    if (!rows.length && s.featured_brands_list) {
        rows = String(s.featured_brands_list)
            .split(/[\n,]+/).map(v => v.trim()).filter(Boolean)
            .map(name => ({ name, image: '' }));
    }
    _brandRows = rows.length ? rows : [];
    brandRenderRows();
}

function brandRenderRows() {
    const wrap = document.getElementById('brandRowsWrap');
    if (!wrap) return;
    wrap.innerHTML = '';
    _brandRows.forEach((brand, idx) => {
        ...
        row.innerHTML = `
            ...
            <!-- Name input -->
            <input type="text" value="${brand.name || ''}" placeholder="Brand name"
                style="flex:1;border:1px solid var(--admin-line,#e5e7eb);border-radius:7px;padding:8px 12px;font-size:13px;background:var(--admin-bg,#fff);color:var(--admin-text,#111);"
                oninput="_brandRows[${idx}].name = this.value; brandSyncJSON();">
            <!-- Image URL (editable) -->
            <input type="text" value="${brand.image || ''}" placeholder="/uploads/brands/logo.png  or paste URL"
                style="flex:2;border:1px solid var(--admin-line,#e5e7eb);border-radius:7px;padding:8px 12px;font-size:12px;background:var(--admin-bg,#fff);color:var(--admin-text-muted,#6b7280);"
                oninput="_brandRows[${idx}].image = this.value; brandRefreshPreview(${idx}); brandSyncJSON();">
            <!-- Remove -->
            <button type="button" onclick="brandRemoveRow(${idx})" title="Remove brand" style="
                background:none;border:none;cursor:pointer;color:#ef4444;font-size:18px;padding:4px;line-height:1;
            ">✕</button>
        `;
...
function brandAddRow() {
    _brandRows.push({ name: '', image: '' });
    brandRenderRows();
    // Focus the new name input
    setTimeout(() => {
        const inputs = document.querySelectorAll('#brandRowsWrap input[type=text]');
        if (inputs.length >= 2) inputs[inputs.length - 2].focus();
    }, 50);
}

function brandRemoveRow(idx) {
    _brandRows.splice(idx, 1);
    brandRenderRows();
}

function brandRefreshPreview(idx) {
    const preview = document.getElementById(`brand-preview-${idx}`);
    if (!preview) return;
    const url = _brandRows[idx]?.image || '';
    if (url) {
        const src = url.startsWith('/uploads') ? '..' + url : url;
        preview.innerHTML = `<img src="${src}" style="max-height:40px;max-width:60px;object-fit:contain;">`;
    } else {
        preview.innerHTML = '🖼️';
    }
}

function brandSyncJSON() {
    const valid = _brandRows.filter(b => b.name || b.image);
    const jsonEl = document.getElementById('featured_brands_data');
    const legacyEl = document.getElementById('featured_brands_list');
    if (jsonEl) jsonEl.value = JSON.stringify(valid);
    if (legacyEl) legacyEl.value = valid.map(b => b.name).filter(Boolean).join(', ');
}

async function brandUploadImage(idx, input) {
    if (!input.files || !input.files[0]) return;
    const file = input.files[0];
    const preview = document.getElementById(`brand-preview-${idx}`);
    if (preview) preview.innerHTML = '<span style="font-size:11px;color:#6b7280">Uploading…</span>';
...
async function saveSettings() {
    ...
    [1,2,3,4,5,6].forEach(i => {
        const file = document.getElementById(`review_${i}_photo_file`)?.files[0];
        if (file) fd.append(`review_${i}_photo`, file);
    });
