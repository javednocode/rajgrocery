...
.btn-apply { padding:7px 16px; background:#2563EB; color:white; border:none; border-radius:7px; font-size:13px; font-weight:700; cursor:pointer; }
.btn-apply:hover { background:#3a2166; }
.btn-clear-sel { padding:7px 12px; background:transparent; color:#6B7280; border:1.5px solid #D1D5DB; border-radius:7px; font-size:12px; cursor:pointer; }
...
.btn-modal-cancel { padding:9px 18px; background:transparent; color:var(--admin-text-muted); border:1.5px solid var(--admin-border); border-radius:8px; cursor:pointer; font-size:13px; font-weight:600; }
.btn-modal-confirm { padding:9px 20px; background:#2563EB; color:white; border:none; border-radius:8px; cursor:pointer; font-size:13px; font-weight:700; }
...
<div class="ptb">
    ...
    <div class="ptb-right">
        <a href="product-edit.php" id="addProductBtn" class="btn btn-primary">+ Add Product</a>
    </div>
</div>
...
function renderPager(pag, page) {
    ...
    if (tp <= 7) { for (let i=1;i<=tp;i++) html += mkLink(i); }
    else {
        html += mkLink(1);
...
async function executeBulkAction() {
    ...
    if (pendingAction === 'set_category') {
        ...
        if (sourceVal) { body.source_category_id = sourceVal; }
        else if (!confirm('No source category selected.\nProducts will be ADDED to the destination without removing from any other category.\nProceed anyway?')) return;
    }
    closeBulkModal();
    try {
        ...
        clearSelection();
        loadProducts(currentPage);
        showAlert('Done — selected product(s) updated');
    } catch(e) {
...
(async function init() {
    // Make sure checkAuth has run and token is available
    // checkAuth() is called at bottom of page, but we run before that
    // So explicitly check here:
    if (typeof checkAuth === 'function') checkAuth();
    const token = localStorage.getItem('admin_token') ||
        (document.cookie.match(/admin_token=([^;]+)/) || [])[1] || '';
    if (!token) {
        // No token — wait briefly for checkAuth to redirect us, or redirect now
