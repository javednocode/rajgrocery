...
function getAllGatewaysAdmin(PDO $db): void {
    ...
    // Auto-create table if missing (for backward compatibility)
    try {
        $db->query("SELECT 1 FROM payment_gateways LIMIT 1");
        ...
        $db->exec("
            CREATE TABLE IF NOT EXISTS `payment_gateways` (
              ...
              PRIMARY KEY (`id`),
              UNIQUE KEY `site_gw` (`site_id`, `gateway_key`),
            ...
            (1, 'stripe', 'Stripe (Credit Cards)', 0, 2),
    ...
    foreach ($gateways as &$gw) {
        ...
        foreach (['secret_key', 'webhook_secret', 'private_key', 'client_secret', 'key_secret'] as $sensitiveKey) {
