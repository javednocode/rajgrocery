...
function getReportsSummary(PDO $db): void {
    ...
    // Low stock count
    $stockStmt = $db->query("SELECT COUNT(*) FROM products WHERE is_active = 1 AND stock <= COALESCE(low_stock_threshold, 5)");
    $lowStock = $stockStmt->fetchColumn();

    // Products count
    $prodStmt = $db->query("SELECT COUNT(*) FROM products WHERE is_active = 1");
...
function getCustomersReport(PDO $db): void {
    ...
    $newStmt = $db->prepare("SELECT DATE(created_at) AS day, COUNT(*) AS new_customers
        FROM customers WHERE created_at BETWEEN :f AND :t
        GROUP BY DATE(created_at) ORDER BY day");
    $newStmt->execute([':f' => $from, ':t' => $to . ' 23:59:59']);
...
function getConversionReport(PDO $db): void {
    ...
    $payStmt = $db->query("SELECT payment_status, COUNT(*) AS count
