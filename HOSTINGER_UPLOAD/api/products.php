...
function bulkProductAction($db) {
    ...
    switch ($action) {
        ...
        case 'set_category':
            ...
            // Remove all existing categories then assign the new one
            $db->exec("DELETE FROM product_categories WHERE product_id IN ($placeholders)");
            $stmt = $db->prepare("INSERT INTO product_categories (product_id, category_id) VALUES (:pid, :cid)");
            foreach ($ids as $pid) { $stmt->execute([':pid' => $pid, ':cid' => $categoryId]); }
            successResponse(['affected' => count($ids)], 'Category assigned to products');
            break;
        case 'add_category':
            $categoryId = intval($data['category_id'] ?? 0);
            if (!$categoryId) errorResponse('Category ID required', 400);
