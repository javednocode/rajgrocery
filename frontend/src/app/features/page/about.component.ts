...
@Component({
  ...
  <!-- Values -->
  <section class="ab-values section" style="background:#F1EADD">
    <div class="container">
  ...
  <section class="section">
    <div class="container ab-promise">
      <div class="ab-promise-text">
        ...
        <ul class="ab-promise-list">
          @for (p of promises; track p) {
            <li>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M5 13l4 4 10-11" stroke="#1F4D3A" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
              {{ p }}
            </li>
          }
        </ul>
      ...
      <div class="ab-promise-cards">
        <div class="ab-prom-card" style="background:linear-gradient(135deg,#211D16,#37322A)">
          <div style="font-size:2.5rem;margin-bottom:12px">🚚</div>
          <strong>Free Delivery</strong>
          <p>On orders over {{ settings.get('currency_symbol','€') }}{{ settings.get('shipping_free_above','50') }}</p>
        </div>
        <div class="ab-prom-card" style="background:linear-gradient(135deg,#C4622D,#D98D5F)">
          <div style="font-size:2.5rem;margin-bottom:12px">🌶️</div>
          <strong>Authentic Products</strong>
          <p>Sourced directly from trusted suppliers</p>
        </div>
        <div class="ab-prom-card" style="background:linear-gradient(135deg,#1F4D3A,#5F8F76)">
          <div style="font-size:2.5rem;margin-bottom:12px">💬</div>
  ...
  .sec-eyebrow { display: inline-block; font-family: 'Manrope', sans-serif; font-size: 11px; font-weight: 700; letter-spacing: .18em; text-transform: uppercase; color: #C4622D; margin-bottom: 10px; }
  .sec-title { font-family: 'Fraunces', Georgia, serif; font-size: clamp(1.5rem, 3vw, 2.2rem); font-weight: 400; color: #211D16; margin-bottom: 12px; line-height: 1.2; }
  .sec-sub { font-size: 15px; color: #7C7466; line-height: 1.7; }

  /* HERO */
  .ab-hero { background: linear-gradient(135deg, #211D16 0%, #37322A 100%); padding: 72px 0 80px; position: relative; overflow: hidden; }
  .ab-crumbs { display: flex; align-items: center; gap: 8px; font-size: 13px; color: rgba(255,255,255,.4); margin-bottom: 16px; }
  .ab-crumbs a { color: rgba(255,255,255,.65); transition: color .2s; } .ab-crumbs a:hover { color: #C4622D; }
  .ab-crumbs i { font-style: normal; opacity: .35; }
  .ab-eyebrow { display: inline-block; font-family: 'Manrope', sans-serif; font-size: 11px; font-weight: 700; letter-spacing: .18em; text-transform: uppercase; color: #C4622D; margin-bottom: 14px; }
  ...
  .ab-cta { display: inline-flex; align-items: center; gap: 8px; background: #C4622D; color: #fff; padding: 13px 26px; border-radius: 999px; font-family: 'Manrope', sans-serif; font-size: 14px; font-weight: 800; box-shadow: 0 6px 20px rgba(196,98,45,.28); transition: all .25s; }
  .ab-cta:hover { background: #A94E20; transform: translateY(-1px); }
  .ab-story-img { position: relative; }
  .ab-img-card { background: linear-gradient(135deg, #37322A 0%, #211D16 100%); border-radius: 24px; height: 380px; display: flex; align-items: center; justify-content: center; position: relative; overflow: hidden; }
  .ab-img-emoji { font-size: 7rem; }
  .ab-img-overlay { position: absolute; bottom: 0; left: 0; right: 0; background: rgba(28,25,19,.8); padding: 20px 24px; backdrop-filter: blur(8px); }
  ...
  .ab-img-float { position: absolute; top: 24px; right: -20px; background: #fff; border-radius: 16px; padding: 14px 18px; display: flex; flex-direction: column; align-items: center; box-shadow: 0 8px 28px rgba(28,25,19,.2); }
  .ab-img-float span { font-size: 1.4rem; }
  .ab-img-float strong { font-family: 'Manrope', sans-serif; font-size: 16px; font-weight: 800; color: #C4622D; }
  .ab-img-float em { font-style: normal; font-size: 11px; color: #ABA394; font-family: 'Manrope', sans-serif; }

  /* VALUES */
  .ab-values-grid { display: grid; grid-template-columns: repeat(4,1fr); gap: 20px; }
  .ab-value-card { background: #fff; border: 1.5px solid #E8E1D2; border-radius: 20px; padding: 28px 22px; transition: all .3s; }
  .ab-value-card:hover { transform: translateY(-5px); box-shadow: 0 14px 36px rgba(28,25,19,.1); border-color: rgba(196,98,45,.25); }
  .ab-value-icon { font-size: 2.2rem; margin-bottom: 14px; }
  .ab-value-card h3 { font-family: 'Fraunces', Georgia, serif; font-size: 1.05rem; font-weight: 400; color: #211D16; margin-bottom: 8px; }
  .ab-value-card p { font-size: 13.5px; color: #7C7466; line-height: 1.6; margin: 0; }

  /* STATS BAND */
  .ab-stats-band { background: #C4622D; padding: 40px 0; }
  ...
  /* CTA BAND */
  .ab-cta-band { background: linear-gradient(135deg, #211D16 0%, #37322A 100%); padding: 72px 0; text-align: center; }
  ...
  .ab-cta-primary { background: #C4622D; color: #fff; padding: 14px 32px; border-radius: 999px; font-family: 'Manrope', sans-serif; font-size: 15px; font-weight: 800; box-shadow: 0 6px 20px rgba(196,98,45,.3); transition: all .25s; }
  .ab-cta-primary:hover { background: #A94E20; transform: translateY(-1px); }
