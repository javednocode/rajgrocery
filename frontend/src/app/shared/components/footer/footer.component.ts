...
@Component({
  ...
  <footer class="ft" [class.ft-revealed]="revealed()">
    ...
    <div class="ft-wave-wrap" aria-hidden="true">
      <svg class="ft-wave ft-wave-1" viewBox="0 0 1440 80" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M0,40 C180,80 360,0 540,40 C720,80 900,0 1080,40 C1260,80 1350,20 1440,40 L1440,80 L0,80 Z" fill="currentColor"/>
      </svg>
      <svg class="ft-wave ft-wave-2" viewBox="0 0 1440 80" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M0,50 C200,10 400,70 600,40 C800,10 1000,70 1200,40 C1300,25 1380,55 1440,40 L1440,80 L0,80 Z" fill="currentColor"/>
      </svg>
      <svg class="ft-wave ft-wave-3" viewBox="0 0 1440 80" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M0,30 C240,60 480,10 720,45 C960,80 1200,20 1440,50 L1440,80 L0,80 Z" fill="currentColor"/>
      </svg>
    </div>
      ...
      <div class="ft-grid">
        ...
        <div class="ft-brand ft-reveal-item" style="--stagger: 0s">
          <a routerLink="/" class="ft-mark" [attr.aria-label]="settings.get('site_name','Raj Grocery Store') + ' — home'">
            <!-- Spinning coin logo -->
            <div class="ft-coin-wrap">
              <div class="ft-coin">
                <div class="ft-coin-face ft-coin-front">
                  @if (logoUrl()) {
                    <img [src]="logoUrl()" [alt]="settings.get('site_name', 'Raj Grocery Store')" class="ft-logo" loading="lazy" (error)="logoFailed.set(true)" />
                  } @else {
                    <span class="ft-coin-letter">{{ settings.get('site_name','Raj').charAt(0) }}</span>
                  }
                </div>
                <div class="ft-coin-face ft-coin-back">
            ...
            <span class="ft-wordmark">
              ...
              <span class="ft-wordmark-sub">{{ settings.get('site_tagline', 'Indian Grocery · Hong Kong') }}</span>
            </span>
          </a>

          <p class="ft-tagline">{{ settings.get('footer_about', 'Your premium Indian grocery store in Hong Kong — spices, staples, snacks, fresh vegetables and household essentials delivered to your door.') }}</p>

          @if (hasSocialLinks()) {
            <div class="ft-connect">
              <span class="ft-label">Connect with us</span>
              <div class="ft-social-icons">
                @if (settings.get('social_whatsapp')) {
                  <a class="ft-so-btn" [href]="'https://wa.me/' + settings.get('social_whatsapp').replace(/\\D/g,'')" target="_blank" rel="noopener" aria-label="Chat on WhatsApp" title="WhatsApp">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M11.996 0C5.374 0 0 5.373 0 11.996c0 2.117.554 4.102 1.523 5.825L.053 23.948l6.284-1.647A11.938 11.938 0 0011.996 24c6.622 0 11.996-5.373 11.996-11.996S18.618 0 11.996 0zm0 21.818a9.808 9.808 0 01-5.003-1.365l-.359-.214-3.729.977.994-3.635-.235-.374A9.806 9.806 0 012.178 12c0-5.41 4.409-9.819 9.818-9.819 5.41 0 9.819 4.409 9.819 9.819S17.406 21.818 11.996 21.818z"/></svg>
                  </a>
                }
            ...
            <div class="ft-visit">
              ...
              <div class="ft-contact-details">
                ...
                @if (settings.get('contact_address') && !isPlaceholderAddress()) {
                  <div class="ft-detail-row">
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M21 10c0 6-9 12-9 12s-9-6-9-12a9 9 0 0118 0z" stroke="currentColor" stroke-width="1.8"/><circle cx="12" cy="10" r="3" stroke="currentColor" stroke-width="1.8"/></svg>
                ...
                @if (settings.get('site_phone')) {
                  <a class="ft-detail-row ft-detail-link" [href]="'tel:' + settings.get('site_phone').replace(/\\s/g,'')">
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M22 16.9v3a2 2 0 01-2.2 2 19.8 19.8 0 01-8.6-3.1 19.5 19.5 0 01-6-6A19.8 19.8 0 012.1 4.2 2 2 0 014 2h3a2 2 0 012 1.7 12.8 12.8 0 00.7 2.8 2 2 0 01-.5 2.1L8.1 9.9a16 16 0 006 6l1.3-1.2a2 2 0 012.1-.5 12.8 12.8 0 002.8.7A2 2 0 0122 16.9z" stroke="currentColor" stroke-width="1.8"/></svg>
          ...
          <div class="ft-cta-block">
            ...
            @if (settings.get('social_whatsapp')) {
              ...
              <a class="ft-cta-btn" [href]="'tel:' + settings.get('site_phone').replace(/\\s/g,'')">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M22 16.9v3a2 2 0 01-2.2 2 19.8 19.8 0 01-8.6-3.1 19.5 19.5 0 01-6-6A19.8 19.8 0 012.1 4.2 2 2 0 014 2h3a2 2 0 012 1.7 12.8 12.8 0 00.7 2.8 2 2 0 01-.5 2.1L8.1 9.9a16 16 0 006 6l1.3-1.2a2 2 0 012.1-.5 12.8 12.8 0 002.8.7A2 2 0 0122 16.9z" stroke="currentColor" stroke-width="2"/></svg>
                <span>Call the Store</span>
              </a>
            } @else {
              <a class="ft-cta-btn" routerLink="/contact">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M21 11.5a8.38 8.38 0 01-.9 3.8 8.5 8.5 0 01-7.6 4.7 8.38 8.38 0 01-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 01-.9-3.8 8.5 8.5 0 014.7-7.6 8.38 8.38 0 013.8-.9h.5a8.48 8.48 0 018 8v.5z" stroke="currentColor" stroke-width="2"/></svg>
    ...
    <div class="ft-bottom">
      <div class="ft-bottom-container">
        ...
        <div class="ft-secure-badges" aria-label="Supported payment methods">
          ...
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M20 12V8H6a2 2 0 01-2-2c0-1.1.9-2 2-2h12v4"/><path d="M4 6v12c0 1.1.9 2 2 2h14v-4"/><path d="M18 12a2 2 0 00-2 2c0 1.1.9 2 2 2h2v-4"/><circle cx="16" cy="14" r="1" fill="currentColor"/></svg>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 1v22M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>
        </div>
      </div>
    </div>
  </footer>
  `,
  styles: [`
  /* ═══ RAJ GROCERY — FOOTER (Redesigned) ═══
     Light plum theme · animated waves · spinning coin logo */
  ...
  /* Layer 3 — fastest, darkest = the fill */
  .ft-wave-3 {
    color: var(--raj-dark);
    animation: waveShift3 4s ease-in-out infinite;
    z-index: 3;
  }
  ...
  @keyframes waveShift2 {
    0%, 100% { transform: translateX(0) scaleY(1); }
    50%       { transform: translateX(3%) scaleY(.92); }
  }
  ...
  /* ══ COIN LOGO ══ */
  .ft-mark { display: inline-flex; align-items: center; gap: 14px; margin-bottom: 20px; }
  ...
  .ft-coin-front {
    background: linear-gradient(135deg, #fff 0%, var(--raj-warm) 100%);
    border: 2.5px solid var(--raj-line);
  }
  .ft-coin-back {
    background: linear-gradient(135deg, var(--raj-leaf-bg) 0%, #fff 100%);
    border: 2.5px solid var(--raj-line);
  ...
  .ft-wordmark-main {
    ...
    color: var(--raj-ink); letter-spacing: -0.018em;
  }
  .ft-wordmark-sub {
  ...
  .ft-tagline {
    ...
    color: var(--raj-muted); max-width: 36ch; margin: 0;
  }
  ...
  .ft-label {
    ...
    color: var(--raj-faint); margin-bottom: 12px;
  }
  ...
  .ft-col-header-static {
    ...
    text-transform: uppercase; color: var(--raj-ink);
  ...
  .ft-link {
    font-size: 14px; color: var(--raj-muted);
    transition: color .2s, padding-left .2s;
    display: inline-block; position: relative;
  }
  .ft-link:hover { color: var(--raj-ink); padding-left: 9px; }
  .ft-link::before {
  ...
  .ft-link-accent:hover { color: var(--raj-turmeric-dk); }
  ...
  .ft-cta-btn {
    ...
    box-shadow: 0 4px 16px rgba(242,169,59,.25);
  }
  .ft-cta-btn:hover {
  ...
  .ft-credit a { color: var(--raj-muted); transition: color .2s; }
  .ft-credit a:hover { color: var(--raj-turmeric); }

  .ft-bottom-policies { display: flex; align-items: center; gap: 22px; flex-wrap: wrap; }
  .ft-policy-link {
    font-size: 12.5px; color: var(--raj-muted); transition: color .2s;
  }
  .ft-policy-link:hover { color: var(--raj-ink); }

  .ft-secure-badges { display: flex; align-items: center; gap: 13px; color: var(--raj-faint); }
  ...
  @media (max-width: 720px) {
    .ft-container { padding-top: 40px; padding-bottom: 28px; }
    .ft-grid { grid-template-columns: 1fr; gap: 0; }
    .ft-brand { margin-bottom: 30px; }

    .ft-nav { border-bottom: 1px solid var(--raj-line); }
    .ft-col-header {
    ...
    .ft-coin { width: 56px; height: 56px; }
  }

  @media (prefers-reduced-motion: reduce) {
    .ft-wave-1, .ft-wave-2, .ft-wave-3 { animation: none; }
    .ft-coin { animation: none; }
    .ft-coin-wrap::after { animation: none; }
    .ft-reveal-item { opacity: 1; transform: none; transition: none; }
  }
  `]
})
export class FooterComponent implements OnInit, OnDestroy {
  revealed   = signal(false);
  openCol    = signal<string|null>(null);
  ...
  ngOnInit() {
    ...
    this.observer = new IntersectionObserver(
      ([entry]) => { if (entry.isIntersecting) this.revealed.set(true); },
      { threshold: 0.05 }
    );
    this.observer.observe(this.el.nativeElement);

    // Load categories for the shop column
    this.api.getCategories().subscribe({
      next: (r: any) => {
        const rows = Array.isArray(r) ? r : (r?.data ?? r?.categories ?? []);
        this.categories.set(rows.slice(0, 6));
      },
      error: () => {}
    });

    // Load pages for the policy links
    this.api.getPages().subscribe({
      next: (r: any) => {
        const rows = Array.isArray(r) ? r : (r?.data ?? r?.pages ?? []);
        const policy = rows.filter((p: any) =>
          /privacy|terms|refund|shipping/i.test(p.slug || p.title || '')
        );
        this.policyPages.set(policy);
      },
      error: () => {}
    });
  }
  ...
  logoUrl(): string | null {
    ...
    const v = this.settings.get('site_logo');
  ...
  hasContactDetails(): boolean {
    return !!(
      ...
      this.settings.get('contact_address') ||
  ...
  isPlaceholderAddress(): boolean {
    const a = String(this.settings.get('contact_address') || '');
    return /1234|example|placeholder|your.address/i.test(a);
  }

  copyright(): string {
    const raw = this.settings.get('footer_copyright', '');
    if (!raw) return this.settings.get('site_name', 'Raj Grocery Store') + '. All rights reserved.';
    return String(raw).replace(/©\s*\d{4}\s*/g, '').trim();
  }
