...
<form id="productForm" enctype="multipart/form-data">
    <div style="display:grid;grid-template-columns:2fr 1fr;gap:24px;">
        <div>
            ...
            <!-- VARIATIONS SECTION -->
            <div class="card" id="variationsCard" style="display:none;">
...
// Show variations card only in edit mode
if (productId) {
    ...
    try {
        ...
        if (p.images?.length) {
            document.getElementById('existingImages').innerHTML = p.images.map(img =>
                `<div class="preview-item"><img src="../${img.image_path}" alt="${img.alt_text||''}"><button type="button" class="remove-btn" onclick="this.closest('.preview-item').remove()">×</button></div>`
            ).join('');
        }
...
document.getElementById('productForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (!productId && res.data?.id) {
            setTimeout(() => window.location.href = `product-edit.php?id=${res.data.id}`, 800);
        } else {
