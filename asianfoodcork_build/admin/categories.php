        ...
        document.getElementById('categoriesList').innerHTML = flat.map(c => `
            <tr style="opacity:${c.is_active==1?'1':'0.55'}">
                ...
                <td>
                    ...
                    <button class="btn btn-danger btn-sm" onclick="confirmDelete('categories',${c.id},'${c.name.replace(/'/g,"\\'")}')" >Delete</button>
                </td>
            </tr>
...
async function saveCategory() {
    ...
    try {
        // Always use PUT for update — backend handles multipart/form-data on PUT
...
document.getElementById('catName').addEventListener('input', function() { document.getElementById('catSlug').value = generateSlug(this.value); });
loadCategories();
</script>
