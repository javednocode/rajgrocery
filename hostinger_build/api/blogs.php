...
function createBlog($db) {
    // Support _method=PUT override — POST with FormData for edit (PHP can't read $_POST for PUT+multipart)
    $data = !empty($_POST) ? $_POST : getJsonInput();
    if (!empty($data['_method']) && strtoupper($data['_method']) === 'PUT') {
        // Editing existing post via POST override — extract ID from URL
        $uri = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);
        preg_match('#/blogs/(\d+)#', $uri, $m);
        $id = (int)($m[1] ?? 0);
        if (!$id) errorResponse('Blog post ID missing for update', 400);
        updateBlog($db, $id);
        return;
    }
...
function updateBlog($db, $id) {
    // $_POST is populated for POST requests (including our _method=PUT override)
    // For direct PUT requests (non-FormData / JSON), fall back to raw stream
    $data = !empty($_POST) ? $_POST : getJsonInput();
    $title = trim($data['title'] ?? '');
    if (empty($title)) errorResponse('Title is required', 400);
    ...
    if (!empty($_FILES['featured_image']) && $_FILES['featured_image']['error'] === UPLOAD_ERR_OK) {
        ...
        $image = $data['featured_image']; // preserve existing if not uploading new
    }
    $status = $data['status'] ?? 'draft';
